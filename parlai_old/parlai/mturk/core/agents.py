...
class MTurkAgent(Agent):
    ...
    def wait_for_hit_completion(self, agent, timeout=None):
        control_msg = {'text': 'Click \"Done with this HIT\" button below to proceed to the post-survey and finish this HIT.\
                                You must take the post-survey to complete the task.'}
    ...
    def shutdown(self, agent, timeout=None, direct_submit=False):
        ...
        if not (self.hit_is_abandoned or self.hit_is_returned or
            ...
            return self.wait_for_hit_completion(agent, timeout=timeout)
